...
@Component({
  selector: 'app-clearance-sale-items-tile',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './clearance-sale-items-tile.html',
  styleUrl: './clearance-sale-items-tile.css',
})
export class ClearanceSaleItemsTile implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      const q        = query(collection(this.firestore, 'products'), where('section', '==', 'CLEARANCE_SALE'));
      ...
      console.error('[ClearanceSaleItemsTile] Failed to load products:', error);
      this.hasError = true;
    } finally {
  ...
  protected async addToCart(product: ClearanceProduct, event: Event): Promise<void> {
    ...
    } catch (err) { console.error('[ClearanceSaleItemsTile] Failed to add to cart:', err); }
  }

  protected async toggleWishlist(product: ClearanceProduct, event: Event): Promise<void> {
    event.stopPropagation();
    if (!this.authService.isLoggedIn) { this.authService.router.navigate(['/login']); return; }
    try {
    ...
    } catch (err) { console.error('[ClearanceSaleItemsTile] Failed to toggle wishlist:', err); }
  }
