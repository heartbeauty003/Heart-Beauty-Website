...
@Component({
  selector: 'app-trending-now-items-tile',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './trending-now-items-tile.html',
  styleUrl: './trending-now-items-tile.css',
})
export class TrendingNowItemsTile implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      const q        = query(collection(this.firestore, 'products'), where('section', '==', 'TRENDING_NOW'));
      ...
      console.error('[TrendingNowItemsTile] Failed to load products:', error);
      this.hasError = true;
    } finally {
  ...
  protected async addToCart(product: TrendingProduct, event: Event): Promise<void> {
    ...
    } catch (err) { console.error('[TrendingNowItemsTile] Failed to add to cart:', err); }
  }

  protected async toggleWishlist(product: TrendingProduct, event: Event): Promise<void> {
    event.stopPropagation();
    if (!this.authService.isLoggedIn) { this.authService.router.navigate(['/login']); return; }
    try {
    ...
    } catch (err) { console.error('[TrendingNowItemsTile] Failed to toggle wishlist:', err); }
  }
