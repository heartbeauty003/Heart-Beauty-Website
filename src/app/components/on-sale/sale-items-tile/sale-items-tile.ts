...
export class SaleItemsTile implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      const q        = query(collection(this.firestore, 'products'), where('section', '==', 'ON_SALE'));
      ...
      console.error('[SaleItemsTile] Failed to load products:', error);
      this.hasError = true;
    } finally {
  ...
  protected async addToCart(product: SaleProduct, event: Event): Promise<void> {
    ...
    } catch (err) { console.error('[SaleItemsTile] Failed to add to cart:', err); }
  }
  ...
  protected async toggleWishlist(product: SaleProduct, event: Event): Promise<void> {
    ...
    } catch (err) { console.error('[SaleItemsTile] Failed to toggle wishlist:', err); }
  }
