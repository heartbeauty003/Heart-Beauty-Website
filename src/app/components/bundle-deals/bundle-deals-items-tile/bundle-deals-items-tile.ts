...
@Component({
  selector: 'app-bundle-deals-items-tile',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './bundle-deals-items-tile.html',
  styleUrl: './bundle-deals-items-tile.css',
})
export class BundleDealsItemsTile implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      await runInInjectionContext(this.environmentInjector, async () => {
        const q        = query(collection(this.firestore, 'products'), where('section', '==', 'BUNDLE_DEALS'));
      ...
      console.error('[BundleDealsItemsTile] Failed to load products:', error);
      this.hasError = true;
    } finally {
  ...
  protected async addToCart(product: BundleProduct, event: Event): Promise<void> {
    ...
    } catch (err) { console.error('[BundleDealsItemsTile] Failed to add to cart:', err); }
  }

  protected async toggleWishlist(product: BundleProduct, event: Event): Promise<void> {
    event.stopPropagation();
    if (!this.authService.isLoggedIn) { this.authService.router.navigate(['/login']); return; }
    try {
    ...
    } catch (err) { console.error('[BundleDealsItemsTile] Failed to toggle wishlist:', err); }
  }
