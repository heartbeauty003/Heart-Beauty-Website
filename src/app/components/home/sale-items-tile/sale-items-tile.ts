...
@Component({
  selector: 'app-sale-items-tile',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './sale-items-tile.html',
  styleUrl: './sale-items-tile.css',
})
export class SaleItemsTile implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      const q        = query(collection(this.firestore, 'products'), where('section', '==', 'ON_SALE'), limit(5));
      ...
      console.error('[HomeSaleItemsTile] Failed to load products:', error);
      this.hasError = true;
    } finally {
  ...
  protected async addToCart(product: SaleProduct, event: Event): Promise<void> {
    ...
    } catch (err) { console.error('[HomeSaleItemsTile] Failed to add to cart:', err); }
  }

  protected async toggleWishlist(product: SaleProduct, event: Event): Promise<void> {
    event.stopPropagation();
    if (!this.authService.isLoggedIn) { this.authService.router.navigate(['/login']); return; }
    try {
    ...
    } catch (err) { console.error('[HomeSaleItemsTile] Failed to toggle wishlist:', err); }
  }
